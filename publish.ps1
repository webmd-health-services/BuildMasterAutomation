...
$moduleRoot = (Join-Path -Path $PSScriptRoot -ChildPath 'BuildMasterAutomation' -Resolve)
$manifest = Test-ModuleManifest -Path (Join-Path -Path $moduleRoot -ChildPath 'BuildMasterAutomation.psd1' -Resolve)
if( -not $manifest )
{
    return
}
...
if( $filesNotReady )
{
    Write-Error -Message ('There are uncommitted changes under BuildMasterAutomation. Please remove these files or commit them.{0}{1}' -f [Environment]::NewLine,($filesNotReady -join [Environment]::NewLine))
    return
}
